...
class MemoryConsolidator:
    ...
    def consolidate(self, user_id: str) -> int:
        ...
        results = self.episodic_store.collection.get(
            where={"user_id": user_id}, include=["documents", "embeddings", "metadatas"]
        )

        if (
            not results["documents"]
            or len(results["documents"]) < self.repetition_threshold
        ):
            return 0
        ...
        for cluster in clusters:
            ...
            try:
                response = self.adapter.chat(
                    synthesis_prompt, memories=[], model="llama-3.1-8b-instant"
                )
