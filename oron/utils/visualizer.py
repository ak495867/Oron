...
class GraphVisualizer:
    ...
    def render(self, user_id: str):
        ...
        """
        root = Tree(
            f"[bold cyan]Oron Knowledge Graph[/bold cyan] (User: [yellow]{user_id}[/yellow])"
        )
        ...
        for node in self.store.graph.nodes:
            if node.startswith(user_prefix):
                label = self.store.graph.nodes[node].get(
                    "label", node.replace(user_prefix, "")
                )
                subjects[node] = label
        ...
        for node, label in subjects.items():
            ...
            for _, target, data in self.store.graph.out_edges(node, data=True):
                target_label = self.store.graph.nodes[target].get(
                    "label", target.replace(user_prefix, "")
                )
                ...
                subject_tree.add(
                    f"-> [blue]{rel}[/blue] -> [bold]{target_label}[/bold] ([{conf_color}]conf: {conf}[/{conf_color}])"
                )
