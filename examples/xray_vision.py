...
async def run_xray():
    ...
    console.print(Panel.fit("[bold magenta]Oron X-Ray Vision[/bold magenta]\nProving this is a true Cognitive Engine, not a wrapper.", border_style="magenta"))
    ...
    for hit in ep_hits:
        salience = mem.decay.calculate_episodic_salience(hit["metadata"]["timestamp"], 0.5)
        raw_candidates.append({"type": "episodic", "content": hit["content"], "salience": salience, "embedding": mem.embedder.embed_text(hit["content"])})
    
    sem_hits = mem.semantic_store.get_related("xray_user", "vroski") + mem.semantic_store.get_related("xray_user", "java")
    for hit in sem_hits:
        salience = mem.decay.calculate_semantic_salience(hit["confidence"])
        raw_candidates.append({"type": "semantic", "content": f"Fact: {hit['subject']} {hit['relation']} {hit['object']}", "salience": salience, "embedding": mem.embedder.embed_text(f"Fact: {hit['subject']} {hit['relation']} {hit['object']}")})
